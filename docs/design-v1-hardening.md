...
> **状态：历史文档（已实施并超出）。**
> 本文是当时的审计与设计依据，其中「现状」一节描述的缺陷已全部修复；
> 文中划为「二期」的 Tool Calling 与页面操作也已实现（含 risk 分级确认与服务端策略）。
> 当前架构见 [architecture.md](architecture.md)，操作层见 [actions.md](actions.md)，
> 接口契约见 [api.md](api.md)。保留本文用于理解取舍背景，勿作为现状参考。
